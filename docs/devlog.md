...
# DAY 5 — 2026-04-19
Model upgrade from qwen3:8b to qwen3:14b. Investigated gemma3:12b as an alternative — better instruction following and 128k context — but it crashes mid-inference on the Intel Arc iGPU under IPEX-LLM (SYCL backend does not yet fully support gemma3's architecture). Reverted to qwen3:14b which runs stably on iGPU and is a significant intelligence improvement over 8b with no compatibility risk.

Scope reduced. `yearly_summary` and `note_cleaner` workflows removed — not worth the maintenance burden at this stage. Project now covers two workflows: `monthly_summary` and `semantic_search`.

Documentation reorganised. `devlog.md`, `modes.md`, `modes_full.md`, `user.md` moved into a new `docs/` folder. `technologies.md` added to document all used technologies — split into a general overview section and a project-specific usage section. `modes.md` rewritten with clearer, more concrete descriptions. `user.md` trimmed and updated with a semantic search preferences section.

Classifier extracted from `manager.py`. Routing logic (`RoutingResult`, `classify()`, `collect_missing_params()`, `prepend_vault_path()`, parameter tables) moved to a dedicated `classifier.py` module. `manager.py` is now a thin CLI entry point only.

Git repository initialised and pushed to GitHub (`GJanos/obsidian_llm`). First commit includes the full project as it stands after today's refactoring session.

Pushed back on the required folder parameter for semantic_search, since that would defeat the purpose of whole-vault search. Agreed with Claude on keeping it optional and defaulting to vault root.

Challenged the plan's claim that no LLM calls are needed at search time. Pure vector similarity is fast but encodes query and document independently — it misses intent and nuanced context. Proposed running the top 15–20 vector candidates through qwen3:14b for a second-pass relevance score. Claude confirmed this is the standard retrieval + re-ranking pattern used in production RAG systems, and suggested listwise re-ranking (all candidates in one LLM call) over pointwise scoring (one call per candidate) — the model can compare candidates against each other rather than scoring in isolation, which is both faster and more accurate. Adopted this as the implementation plan.
